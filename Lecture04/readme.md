...
3. **Polynomial Regression and Basis Expansion**
   - Transform features: x → x, x², x³, ...
   - Example equation: y = β₀ + β₁x + β₂x² + β₃x³
...
6. **Elastic Net**
   - Cost function: RSS + λ₁Σβᵢ² + λ₂Σ|βᵢ|
   - Combines benefits of both Ridge and Lasso
   - Two hyperparameters: α (mixing ratio) and λ (strength)
   - α = 0 → Ridge; 0 < α < 1 → Elastic Net; α = 1 → Lasso
...
9. **Regression vs Classification Problems**
   ...
   - Key difference: output space ℝ (continuous) vs. finite set (discrete)
   - Regression output: y ∈ ℝ (e.g., ŷ = 25.7°C)
   - Classification output: y ∈ {0, 1, ..., K-1} (e.g., ŷ = 1 for Spam)
...
10. **Linear Classifier Concepts**
    ...
    - Classification rule: if f(x) > 0 → Class 1; else → Class 0
...
15. **Introduction to Sigmoid Function**
    ...
    - Output: P(y=1|x) = σ(wᵀx + b)
    - Decision threshold: z=0 → σ=0.5
...
16. **Properties of Logistic Function**
    ...
    - Derivative: σ'(z) = σ(z)(1 - σ(z)) (useful for gradient descent)
...
17. **Defining the Logistic Regression Model**
    ...
    - Probability output: P(y=1|x) = σ(wᵀx + b)
...
18. **Maximum Likelihood Estimation (MLE)**
    ...
    - Likelihood function: L(w,b) = ∏ P(yᵢ|xᵢ)
    - Log-likelihood (easier to optimize): ℓ = Σ log P(yᵢ|xᵢ)
    - For binary classification: ℓ = Σ[yᵢ log(p) + (1-yᵢ)log(1-p)]
...
19. **Binary Cross-Entropy Loss**
    ...
    - Average loss over dataset: BCE = -(1/n)Σ[yᵢ log(ŷᵢ) + (1-yᵢ)log(1-ŷᵢ)]
    - Gradient: ∂L/∂w = (ŷ - y)x (elegant and simple!)

20. **Applying Gradient Descent**
    - Iterative optimization algorithm
    - Update rule: w ← w - η∇L(w)
    - For logistic regression: w ← w - η(ŷ - y)x
...
22. **Softmax Regression**
    ...
    - Softmax function: P(y=k|x) = exp(zₖ) / Σⱼ exp(zⱼ)
    - Properties: outputs sum to 1, differentiable, interpretable
    - Each class has its own weight vector wₖ
...
23. **Categorical Cross-Entropy**
    ...
    - Gradient: ŷₖ - yₖ (elegant and simple!)

24. **Regularized Logistic Regression**
    - Prevent overfitting with penalty terms
    - L2 (Ridge): Loss + λΣwᵢ² (smooth coefficient shrinkage)
    - L1 (Lasso): Loss + λΣ|wᵢ| (feature selection)
...
1. Sigmoid function: `σ(z) = 1 / (1 + exp(-z))`
